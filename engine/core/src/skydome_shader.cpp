...
bool SkyDomeShader::Init(ID3D11Device* device, HWND hwnd)
{
	...
	LPCSTR vsFilename = (LPCSTR)"../engine/core/inc/shaders/skydome.vs";
    LPCSTR psFilename = (LPCSTR)"../engine/core/inc/shaders/skydome.ps";
...
bool SkyDomeShader::InitializeShader(ID3D11Device* device, HWND hwnd, LPCSTR* vsFilename, LPCSTR* psFilename)
{
	HRESULT result;
	...
	gradientBufferDesc.ByteWidth = sizeof(gradientBuffer_t);
...
void SkyDomeShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCSTR* shaderFilename)
{
	char* compileErrors;
	unsigned long i;
	size_t bufferSize;
										...
										D3DXMATRIX projectionMatrix, D3DXVECTOR4 apexColor, D3DXVECTOR4 centerColor)
{
	...
	gradientBuffer_t* gradientBufPtr;
	...
	// Get a pointer to the data in the constant buffer.
	gradientBufPtr = (gradientBuffer_t*)mappedResource.pData;

	// Copy the gradient color variables into the constant buffer.
	gradientBufPtr->apexColor = apexColor;
	gradientBufPtr->centerColor = centerColor;
