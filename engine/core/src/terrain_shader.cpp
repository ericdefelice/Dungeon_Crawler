/*!
  ...
  @brief
  Functionality for the world terrain shader.
...
bool TerrainShader::Init(ID3D11Device* device, HWND hwnd)
{
    ...
    LPCSTR vsFilename = (LPCSTR)"../engine/core/inc/shaders/terrain.vs";
    LPCSTR psFilename = (LPCSTR)"../engine/core/inc/shaders/terrain.ps";
...
bool TerrainShader::InitializeShader(ID3D11Device* device, HWND hwnd, LPCSTR* vsFilename, LPCSTR* psFilename)
{
	HRESULT result;
...
void TerrainShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCSTR* shaderFilename)
{
	char* compileErrors;
	uint32 bufferSize, i;
	ofstream fout;
										...
										ID3D11ShaderResourceView* grassTexture, ID3D11ShaderResourceView* slopeTexture, ID3D11ShaderResourceView* rockTexture)
{
	...
	// Get a pointer to the data in the constant buffer.
	lightBufPtr = (lightBuffer_t*)mappedResource.pData;
