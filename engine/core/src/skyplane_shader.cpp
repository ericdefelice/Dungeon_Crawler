/*!
  ...
  @brief
  Functionality for the shader that renders the sky plane textures.
...
bool SkyPlaneShader::Init(ID3D11Device* device, HWND hwnd)
{
	...
	LPCSTR vsFilename = (LPCSTR)"../engine/core/inc/shaders/skyplane.vs";
	LPCSTR psFilename = (LPCSTR)"../engine/core/inc/shaders/skyplane.ps";
...
bool SkyPlaneShader::InitializeShader(ID3D11Device* device, HWND hwnd, LPCSTR* vsFilename, LPCSTR* psFilename)
{
	HRESULT result;
	...
	skyBufferDesc.ByteWidth = sizeof(skyBuffer_t);
...
void SkyPlaneShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCSTR* shaderFilename)
{
	char* compileErrors;
	unsigned long i;
	size_t bufferSize;
											  ...
											  ID3D11ShaderResourceView* perturbTexture, float translation, float scale, float brightness)
{
	...
	// Get a pointer to the data in the constant buffer.
	skyBufPtr = (skyBuffer_t*)mappedResource.pData;

	// Copy the data into the sky constant buffer.
	skyBufPtr->translation = translation;
	skyBufPtr->scale = scale;
	skyBufPtr->brightness = brightness;
	skyBufPtr->padding = 0.0f;
